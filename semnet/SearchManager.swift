...
class SearchManager: NSObject {
    ...
    func getAllTags(callback: @escaping (Bool,Array<SemanticLabel>) -> ()) {
        ...
        Alamofire.request(searchAllTagsEndpoint, method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                ...
                for anItem in dataList as! [Dictionary<String, AnyObject>] {
                    let label = anItem["label"] as! String
                    let clazz = anItem["clazz"] as! String
                    let count = anItem["count"] as! Int
                    
                    var semLabel = SemanticLabel(tag: label, clazz: clazz)
                    semLabel.count = count
                    
                    labelArray.append(semLabel)
